...
func TestExpandServerRoot(t *testing.T) {
	...
	in := types.MCPServer{
		...
		Env:     map[string]string{"ROOT": "${NIB_PLUGIN_ROOT}", "X": "y"},
	}
	...
	if in.Command != "${NIB_PLUGIN_ROOT}/bin/echo-mcp" || in.Args[1] != "${CLAUDE_PLUGIN_ROOT}/data" {
