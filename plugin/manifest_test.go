...
func TestValidate(t *testing.T) {
	cases := []struct {
		...
		{"nib constraint met", Manifest{Name: "a", NibVersion: ">=0.5.0"}, "0.9.0", false},
		{"nib constraint unmet", Manifest{Name: "a", NibVersion: ">=1.0.0"}, "0.9.0", true},
		{"dev build skips constraint", Manifest{Name: "a", NibVersion: ">=1.0.0"}, "", false},
		{"prefixed v version", Manifest{Name: "a", NibVersion: ">=0.5.0"}, "v0.9.0", false},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			err := c.m.Validate(c.nib)
